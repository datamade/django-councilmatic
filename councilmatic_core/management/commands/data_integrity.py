...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            message = "ConnectionError: Unable to connect to Solr at {} when running the data integrity check. Is Solr running?".format(
                solr_url
            )
        ...
        if solr_index_count != councilmatic_count:
            message = "Solr index has {solr} entites. The Councilmatic database has {councilmatic} entities. That's a problem!".format(
                solr=solr_index_count, councilmatic=councilmatic_count
            )
            logger.error(message)
            print(". . . . .\n")
        ...
        logger.info(
            "Good news! Solr index has {solr} entites. The Councilmatic database has {councilmatic} entities.".format(
                solr=solr_index_count, councilmatic=councilmatic_count
            )
        )
        print(". . . . .\n")
