...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.local_now = pytz.timezone(settings.TIME_ZONE).localize(
            datetime.datetime.now()
        )

        self.bills_on_upcoming_agendas = EventRelatedEntity.objects.filter(
            bill__isnull=False, agenda_item__event__start_date__gte=self.local_now
    ...
    def handle(self, *args, **options):
        ...
        success_message = (
            "Removed {} document(s) from the councilmatic-document-cache".format(
                len(aws_keys)
            )
        )
    ...
    def _get_bill_versions(self, window_start):
        ...
        return BillVersionLink.objects.filter(recently_updated | upcoming).values_list(
            "url", flat=True
        )
